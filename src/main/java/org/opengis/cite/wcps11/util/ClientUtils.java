...
import org.opengis.cite.wcps11.ReusableEntityFilter;
...
public class ClientUtils {
     ...
     * generic json ("application/json") is preferred.
     *
    ...
    public static ClientRequest buildGetRequest(URI endpoint,
        ...
        if (null != qryParams) {
            for (Map.Entry<String, String> param : qryParams.entrySet()) {
                uriBuilder.queryParam(param.getKey(), UriComponent.encode(param.getValue(), UriComponent.Type.QUERY));
            }
        }
     ...
     *
     * @param endpoint A URI indicating the target resource.
     * @param parts A Map containing form parts; values may be String or File objects.
     * @param mediaTypes A list of acceptable media types for the response; if
     * not specified, generic json ("application/json") is preferred.
     *
     * @return A ClientRequest object.
     */
    public static ClientRequest buildMultipartRequest(URI endpoint,
            Map<String, Object> parts, MediaType... mediaTypes) {
        FormDataMultiPart multiPart = new FormDataMultiPart();
        for (Map.Entry<String, Object> entry : parts.entrySet()) {
            String name = entry.getKey();
            Object value = entry.getValue();
            if (value instanceof File) {
                multiPart.bodyPart(new FileDataBodyPart(name, (File) value));
            } else if (value != null) {
                multiPart.field(name, value.toString());
            }
        }
        ClientRequest.Builder reqBuilder = ClientRequest.create()
        ...
        return reqBuilder.build(endpoint, HttpMethod.POST);
    }
